...
class VectorStore:
    def __init__(self, model_name='intfloat/multilingual-e5-large', data_dir='/kaggle/input/uet-rag-data', output_dir='/kaggle/working', device=None, index_type='flat_l2'):
        """
        Khởi tạo vector store với FAISS.
        Args:
            model_name (str): Tên model để encode query (nếu cần).
    ...
    def load_model(self, offline_path=None):
        ...
        try:
            if offline_path and os.path.exists(offline_path):
                print(f"Loading model from {offline_path}")
    ...
    def load_embeddings(self, json_path=None, npy_path=None):
        ...
        if json_path and os.path.exists(json_path):
            ...
            # Nếu không có JSON, giả sử chunks được lưu riêng
    ...
    def run(self, embedding_json=None, embedding_npy=None, offline_model_path=None, queries=None):
...
if __name__ == "__main__":
    ...
    # Chạy quy trình
    vector_store.run(
        embedding_json='/kaggle/input/uet-rag-data/embeddings.json',
